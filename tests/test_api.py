...
class TestAPIEndpoints:
    ...
    async def test_submit_task_endpoint(
        self, test_client: httpx.AsyncClient, sample_task: Dict[str, Any]
    ):
    ...
    async def test_get_task_status_endpoint(
        ...
        submit_response = await test_client.post(
            "/tasks", json=sample_task, headers=headers
        )
    ...
    async def test_execute_workflow_endpoint(
        self, test_client: httpx.AsyncClient, sample_workflow: Dict[str, Any]
    ):
    ...
    async def test_nonexistent_agent(self, test_client: httpx.AsyncClient):
        ...
        headers = {"Authorization": "Bearer valid_token"}
        response = await test_client.get(
            "/agents/nonexistent_agent_id", headers=headers
        )
        assert response.status_code == 404
