...
class AMASOrchestrator:
    ...
    async def execute_task(
        ...
        try:
            ...
            optimization = await intelligence_manager.optimize_task_before_execution(
                task_data
            )
            ...
            self.logger.info(
                f"📊 Predicted success: {task_prediction.success_probability:.2f}"
            )

            # Execute task with selected agents
            result = await self._execute_with_agents(
                task_id, task_type, target, parameters or {}, optimal_agents
            )
    ...
    def _create_agent_prompt(
        self, task_type: str, target: str, parameters: Dict[str, Any], agents: List[str]
    ) -> str:
...
"""
    ...
    def _analyze_agent_performance(
        self, agents: List[str], response: str
    ) -> Dict[str, Any]:
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents": {
                ...
                "ready": len(
                    [a for a in self.agents.values() if a["status"] == "ready"]
                ),
...
if __name__ == "__main__":
    ...
    async def main():
        if args.status:
            ...
            print(
                f"  Agents: {status['agents']['ready']}/{status['agents']['total']} ready"
            )
            print(
                f"  Providers: {status['providers']['enabled']}/{status['providers']['total_configured']} enabled"
            )
            print(
                f"  Tasks: {status['tasks']['completed']} completed, {status['tasks']['active']} active"
            )
