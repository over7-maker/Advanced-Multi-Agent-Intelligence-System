...
class ConfigManager:
    ...
    def export_config(self, file_path: str):
        ...
        try:
            ...
            self.console.print(
                f"✅ Configuration exported to {file_path}", style="green"
            )
    ...
    def import_config(self, file_path: str):
        ...
        try:
            ...
            self.console.print(
                f"✅ Configuration imported from {file_path}", style="green"
            )
    ...
    def display_config_summary(self):
        ...
        enabled_agents = [
            aid for aid, config in agents.items() if config.get("enabled", True)
        ]
        config_table.add_row(
            "Agents", f"Enabled: {len(enabled_agents)}/{len(agents)} agents"
        )
    ...
    def validate_config(self) -> List[str]:
        ...
        for agent_id, config in agents.items():
            ...
            if (
                not isinstance(config.get("priority"), int)
                or config.get("priority", 0) < 1
            ):
                issues.append(f"Agent {agent_id}: 'priority' must be positive integer")
            if (
                not isinstance(config.get("max_tasks"), int)
                or config.get("max_tasks", 0) < 1
            ):
        ...
        if (
            not isinstance(nlp.get("confidence_threshold"), (int, float))
            or not 0 <= nlp.get("confidence_threshold", 0) <= 1
        ):
