...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    def display_agent_status(self, agents_data: Dict[str, Any]):
        ...
        for agent_id, agent_info in agents.items():
            status_icon = "🟢" if agent_info.get("status") == "idle" else "🟡" if agent_info.get("status") == "busy" else "🔴"
            ...
            agent_table.add_row(
                ...
                agent_info.get('last_activity', 'Unknown')[:19] if agent_info.get('last_activity') else 'Unknown'
            )
    ...
    def display_task_progress(self, task_id: str, progress_data: Dict[str, Any]):
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"Executing {progress_data.get('intent', 'task')}...",
                total=100
            )
            ...
            if agents:
                ...
                progress.update(task, description=f"Executing {progress_data.get('intent', 'task')}... {agent_text}")
    ...
    def _create_task_summary_table(self, task_id: str, results: Dict[str, Any]) -> Table:
        ...
        table.add_row("Intent", results.get("intent", "unknown").replace("_", " ").title())
    ...
    def display_task_history(self, history_data: Dict[str, Any]):
        ...
        for task in sorted_tasks[:20]:  # Show last 20 tasks
            ...
            history_table.add_row(
                ...
                duration,
                task.get("created_at", "Unknown")[:19] if task.get("created_at") else "Unknown"
            )
    ...
    def _display_history_summary(self, history_data: Dict[str, Any]):
        ...
        completed_with_duration = [
            t for t in tasks 
            if t.get("status") == "completed" and isinstance(t.get("duration"), (int, float))
        ]
        avg_duration = 0
