...
class TaskManager:
    ...
    async def _cleanup_old_tasks(self):
        ...
        while True:
            try:
                cutoff_date = datetime.now() - timedelta(days=30)  # Keep tasks for 30 days
                tasks_to_remove = []
    ...
    def get_tasks(self, filter_criteria: Optional[TaskFilter] = None) -> List[Task]:
        ...
        if filter_criteria.intent:
            tasks = [t for t in tasks if filter_criteria.intent.lower() in t.intent.lower()]
        
        if filter_criteria.target:
            tasks = [t for t in tasks if filter_criteria.target.lower() in t.target.lower()]
        ...
        if filter_criteria.tags:
            tasks = [t for t in tasks if any(tag in t.tags for tag in filter_criteria.tags)]
        
        if filter_criteria.agents:
            tasks = [t for t in tasks if any(agent in t.agents_involved for agent in filter_criteria.agents)]
    ...
    def update_task(self, task_id: str, updates: Dict[str, Any]) -> bool:
        ...
        elif updates.get("status") in [TaskStatus.COMPLETED, TaskStatus.FAILED, TaskStatus.CANCELLED]:
    ...
    def _update_stats(self):
        ...
        total_completed = self.stats["completed_tasks"] + self.stats["failed_tasks"] + self.stats["cancelled_tasks"]
        
        if total_completed > 0:
            self.stats["success_rate"] = self.stats["completed_tasks"] / total_completed
        
        # Calculate average duration
        completed_tasks = [t for t in self.tasks.values() if t.status == TaskStatus.COMPLETED and t.duration > 0]
        if completed_tasks:
    ...
    def export_tasks(self, file_path: str, filter_criteria: Optional[TaskFilter] = None):
        ...
        self.console.print(f"✅ Exported {len(tasks)} tasks to {file_path}", style="green")
...
class TaskExecutor:
    ...
    async def execute(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            await asyncio.sleep(min(task.timeout_seconds / 10, 10))  # Scale down for demo
    ...
    def _generate_results(self, task: Task) -> Dict[str, Any]:
        ...
        if "security" in intent.lower():
            ...
            base_results.update({
                "code_quality_score": 80 + (hash(target) % 20),  # Random score 80-100
                "metrics": {
