...
class IntentClassifier:
    ...
    def _initialize_intent_patterns(self):
        ...
        self.intent_patterns = {
            "security_scan": {
                ...
                "keywords": ["scan", "security", "vulnerability", "audit", "penetration", "check"],
                "weight": 1.0,
            ...
            "code_analysis": {
                ...
                "keywords": ["code", "analyze", "review", "quality", "function", "class", "method"],
                "weight": 0.9,
            ...
            "intelligence_gathering": {
                ...
                "keywords": ["research", "investigate", "intelligence", "osint", "gather", "find"],
                "weight": 0.95,
            ...
            "performance_monitoring": {
                ...
                "keywords": ["monitor", "performance", "optimize", "speed", "memory", "cpu"],
                "weight": 0.85,
            ...
            "documentation_generation": {
                ...
                "keywords": ["document", "docs", "write", "generate", "create", "documentation"],
                "weight": 0.8,
            ...
            "testing_coordination": {
                ...
                "keywords": ["test", "testing", "qa", "quality", "assurance", "coverage"],
                "weight": 0.9,
            ...
            "threat_analysis": {
                ...
                "keywords": ["threat", "risk", "danger", "attack", "malware", "vulnerability"],
                "weight": 1.0,
                "agents": ["security_expert", "intelligence_gathering"]
            },
            "incident_response": {
                ...
                "keywords": ["incident", "emergency", "respond", "handle", "fix", "resolve"],
                "weight": 1.0,
                "agents": ["security_expert", "intelligence_gathering", "integration_manager"]
            },
            "system_health": {
                "patterns": [
                    r"system\s+health",
    ...
    async def classify_intent(self, command: str, nlp_result: Dict[str, Any], context_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            intent_scores = self._apply_context_boosts(intent_scores, entities, context_history)
            
            # Apply pattern matching
            intent_scores = self._apply_pattern_matching(intent_scores, processed_command)
            ...
            # Generate reasoning
            reasoning = self._generate_reasoning(intent_name, confidence, entities, context_history)
            
            # Get suggested agents
            suggested_agents = self.intent_patterns.get(intent_name, {}).get("agents", [])
    ...
    def _apply_context_boosts(self, scores: Dict[str, float], entities: List[Dict[str, Any]], context_history: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for entity in entities:
            ...
            if "github_repo" in entity_type or "github.com" in entity_text:
                for intent, boost in self.context_rules["github_repo"]["intent_boost"].items():
                    if intent in boosted_scores:
                        boosted_scores[intent] += boost
            
            # URL domain boost
            elif "url" in entity_type or "domain" in entity_type:
                for intent, boost in self.context_rules["url_domain"]["intent_boost"].items():
                    if intent in boosted_scores:
                        boosted_scores[intent] += boost
            
            # File path boost
            elif "file_path" in entity_type:
                for intent, boost in self.context_rules["file_path"]["intent_boost"].items():
                    if intent in boosted_scores:
                        boosted_scores[intent] += boost
        
        # Apply urgent keyword boost
        urgent_keywords = ["urgent", "emergency", "critical", "asap", "immediately", "now"]
        command_words = set(" ".join([e.get("text", "") for e in entities]).lower().split())
        if any(keyword in command_words for keyword in urgent_keywords):
            for intent, boost in self.context_rules["urgent_keywords"]["intent_boost"].items():
                if intent in boosted_scores:
                    boosted_scores[intent] += boost
    ...
    def _apply_pattern_matching(self, scores: Dict[str, float], command: str) -> Dict[str, float]:
        ...
        for intent, data in self.intent_patterns.items():
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    pattern_scores[intent] = min(pattern_scores[intent] + pattern_boost, 1.0)
                    break  # Only apply first matching pattern
        
        return pattern_scores
    
    def _generate_reasoning(self, intent: str, confidence: float, entities: List[Dict[str, Any]], context_history: List[Dict[str, Any]]) -> List[str]:
