...
class ContextManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Context storage
        self.context_history: deque = deque(maxlen=config.get("max_context_length", 100))
    ...
    def add_context(self, command: str, intent: str, target: str, result: Dict[str, Any], 
    ...
    def _analyze_user_pattern(self, recent_context: List[ContextEntry]) -> Optional[str]:
        """Analyze user behavior pattern"""
        if not recent_context:
            return None
    ...
    def _generate_contextual_suggestions(self, command: str, recent_context: List[ContextEntry], 
    ...
    def _update_context_stats(self):
        ...
        if self.context_history:
            ...
            self.context_stats["average_context_size"] = total_size / len(self.context_history)
    ...
    def get_contextual_recommendations(self, session_id: str, current_intent: str) -> List[str]:
        ...
        elif "intelligence" in current_intent.lower() or "research" in current_intent.lower():
            recommendations.extend([
    ...
    def clear_context(self, session_id: Optional[str] = None):
        ...
        if session_id:
            if session_id in self.session_contexts:
                ...
                self.console.print(f"✅ Context cleared for session {session_id}", style="green")
        else:
    ...
    def display_context_summary(self, session_id: Optional[str] = None):
        ...
        stats_table.add_row("Unique Intents", str(len(set(ctx.intent for ctx in context))))
        stats_table.add_row("Unique Targets", str(len(set(ctx.target for ctx in context))))
