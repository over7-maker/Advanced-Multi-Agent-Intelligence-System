...
class RetryConfig:
    ...
    def _is_timeout_error(self, exception: Exception) -> bool:
        ...
        return (
            isinstance(exception, (asyncio.TimeoutError, TimeoutError))
    ...
    def _is_connection_error(self, exception: Exception) -> bool:
        ...
        return (
            ...
            or "network" in str(exception).lower()
            or "unreachable" in str(exception).lower()
        )
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            try:
                ...
                if not self.config.should_retry(e, attempt):
                    self.logger.warning(
                        f"Function {func.__name__} failed on attempt {attempt}, not retrying: {e}"
                    )
                    break
...
HTTP_RETRY_CONFIG = RetryConfig(
    max_attempts=3, base_delay=1.0, max_delay=30.0, condition=RetryCondition.HTTP_ERRORS
)
