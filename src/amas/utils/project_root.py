...
def is_restricted_directory(path: str) -> bool:
    ...
    try:
        ...
        return any(resolved_path.startswith(restricted) for restricted in RESTRICTED_DIRECTORIES)
...
def find_project_root(start_path: Optional[str] = None) -> str:
        ...
        for indicator in PROJECT_ROOT_INDICATORS:
            ...
            try:
                if os.path.exists(indicator_path):
                    ...
                    logger.info(f"Found project root at: {current_path} (indicator: {indicator})")
                    return current_path
    ...
    logger.warning(f"No project root found after traversing {depth} levels from {original_path}")
    ...
    for indicator in fallback_indicators:
        ...
        if os.path.exists(indicator_path) and not is_restricted_file(indicator_path):
            logger.info(f"Using fallback project root: {original_path} (indicator: {indicator})")
            return original_path
    
    # Fallback 2: Use the original directory if it's safe
    if validate_path_security(original_path):
        logger.warning(f"Using original directory as fallback project root: {original_path}")
        return original_path
...
def get_project_root() -> str:
    ...
    if not hasattr(get_project_root, '_cached_root'):
        try:
            ...
            logger.warning(f"Using current working directory as fallback: {get_project_root._cached_root}")
...
def validate_project_structure(project_root: str) -> bool:
    ...
    try:
        ...
        essential_items = [
            'src', 'lib', 'app', 'main.py', 'index.py', 'app.py',
...
def get_relative_path_from_root(file_path: str, project_root: Optional[str] = None) -> str:
    """
