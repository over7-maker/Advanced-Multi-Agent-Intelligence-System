...
class AuthenticationManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.jwt_expiration = config.get('security', {}).get('jwt_expiration', 3600)  # 1 hour
        self.refresh_expiration = config.get('security', {}).get('refresh_expiration', 86400 * 7)  # 7 days
        ...
        self.max_sessions_per_user = config.get('security', {}).get('max_sessions_per_user', 5)
    ...
    async def create_access_token(self, user_id: str, roles: List[str], expires_delta: Optional[timedelta] = None) -> str:
    ...
    async def create_refresh_token(self, user_id: str, expires_delta: Optional[timedelta] = None) -> str:
        ...
        for role in roles:
            if role == "admin":
                permissions.extend([
                    ...
                    "manage:users", "manage:agents", "manage:system"
                ])
            elif role == "user":
                permissions.extend([
                    "read:own", "write:own", "submit:tasks"
                ])
            elif role == "viewer":
                permissions.extend([
                    "read:all"
    ...
    async def check_permission(self, user_permissions: List[str], required_permission: str) -> bool:
        """Check if user has required permission"""
        return required_permission in user_permissions
    
    async def generate_api_key(self, user_id: str, name: str, permissions: List[str]) -> str:
    ...
    async def get_security_events(self, user_id: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
