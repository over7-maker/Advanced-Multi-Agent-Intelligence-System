...
def build_provider_priority() -> List[str]:
    ...
    logger.info(f"Built provider priority order: {providers} ({len(providers)} available)")
    return providers

async def _with_retries(coro_factory, retries: int = DEFAULT_MAX_RETRIES, base_delay: float = DEFAULT_BASE_DELAY):
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:  # Don't sleep on last attempt
                ...
                logger.debug(f"Retry attempt {attempt + 1} failed, waiting {delay:.1f}s: {e}")
...
async def call_cerebras(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    """Call Cerebras using official SDK."""
    key = _env("CEREBRAS_API_KEY")
    if not key:
        raise ProviderError("CEREBRAS_API_KEY not found in environment", "cerebras", "auth")
...
async def call_cerebras_http(messages: List[Dict[str, str]], session: aiohttp.ClientSession, key: str, **kwargs) -> Dict[str, Any]:
    ...
    async with session.post(url, headers=headers, json=body, timeout=DEFAULT_TIMEOUT) as resp:
        if resp.status >= 400:
            error_text = await resp.text()
            error_type = "auth" if resp.status == 401 else "quota" if resp.status == 429 else "error"
            raise ProviderError(f"Cerebras HTTP {resp.status}: {error_text}", "cerebras", error_type)
...
async def call_nvidia(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    ...
    if not OPENAI_AVAILABLE:
        raise ProviderError("OpenAI client not available for NVIDIA", "nvidia", "unavailable")
    ...
    try:
        ...
        error_type = "quota" if "quota" in str(e).lower() else "auth" if "401" in str(e) else "error"
        raise ProviderError(f"NVIDIA failed: {e}", "nvidia", error_type)

async def call_gemini2(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    """Call Gemini 2.0 Flash via Google Generative Language API."""
    key = _env("GEMINI2_API_KEY")
    if not key:
        raise ProviderError("GEMINI2_API_KEY not found in environment", "gemini2", "auth")
    ...
    try:
        ...
        async with session.post(url, headers=headers, json=body, timeout=DEFAULT_TIMEOUT) as resp:
            if resp.status >= 400:
                error_text = await resp.text()
                error_type = "auth" if resp.status == 401 else "quota" if resp.status == 429 else "error"
                raise ProviderError(f"Gemini2 HTTP {resp.status}: {error_text}", "gemini2", error_type)
...
async def call_codestral(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    """Call Codestral using OpenAI-compatible client."""
    key = _env("CODESTRAL_API_KEY")
    if not key:
        raise ProviderError("CODESTRAL_API_KEY not found in environment", "codestral", "auth")
    
    if not OPENAI_AVAILABLE:
        raise ProviderError("OpenAI client not available for Codestral", "codestral", "unavailable")
...
async def call_cohere(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        async with session.post(url, headers=headers, json=body, timeout=DEFAULT_TIMEOUT) as resp:
            if resp.status >= 400:
                error_text = await resp.text()
                error_type = "auth" if resp.status == 401 else "quota" if resp.status == 429 else "error"
                raise ProviderError(f"Cohere HTTP {resp.status}: {error_text}", "cohere", error_type)
...
async def call_chutes(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        async with session.post(url, headers=headers, json=body, timeout=DEFAULT_TIMEOUT) as resp:
            if resp.status >= 400:
                error_text = await resp.text()
                error_type = "auth" if resp.status == 401 else "quota" if resp.status == 429 else "error"
                raise ProviderError(f"Chutes HTTP {resp.status}: {error_text}", "chutes", error_type)
...
async def call_openrouter(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    ...
    if not OPENAI_AVAILABLE:
        raise ProviderError("OpenAI client not available for OpenRouter", "openrouter", "unavailable")
    ...
    try:
        ...
        error_type = "quota" if "quota" in str(e).lower() else "auth" if "401" in str(e) else "error"
        raise ProviderError(f"OpenRouter ({selected_model}) failed: {e}", "openrouter", error_type)

# Placeholder implementations for future providers
async def call_groq2(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    ...
    raise ProviderError("Groq2 adapter not yet implemented - will be added when SDK is available", "groq2", "unavailable")

async def call_groqai(messages: List[Dict[str, str]], session: aiohttp.ClientSession, **kwargs) -> Dict[str, Any]:
    ...
    # TODO: Implement when official SDK/endpoint is available
    raise ProviderError("GroqAI adapter not yet implemented - will be added when SDK is available", "groqai", "unavailable")
...
async def generate(
    ...
    **kwargs
    ...
    logger.info(f"🚀 Starting AI generation with {len(providers)} available providers: {providers}")
            ...
            try:
                ...
                if timeout:
                    result = await asyncio.wait_for(
                        _with_retries(
                            lambda: provider_func(messages, session, temperature=temperature, max_tokens=max_tokens, **kwargs)
                        ),
                        timeout=timeout
                    )
                else:
                ...
                logger.info(f"✅ AI generation successful with {provider_name} in {response_time:.2f}s")
    ...
    logger.error(f"❌ AI generation failed after trying all providers in {response_time:.2f}s")
...
class UniversalAIManager:
    ...
    async def generate(self, prompt: str, system_prompt: str = "", **kwargs) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        if health["status"] == "healthy":
            ...
            if result["success"]:
                print(f"✅ Success with {result['provider_name']}: {result['content'][:100]}...")
            else:
