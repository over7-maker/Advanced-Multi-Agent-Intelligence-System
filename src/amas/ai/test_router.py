...
class TestUniversalRouter(unittest.TestCase):
    ...
    async def test_all_providers_fail(self, mock_priority):
        ...
        with patch("src.amas.ai.router.call_cerebras") as mock_cerebras, patch(
            "src.amas.ai.router.call_nvidia"
        ) as mock_nvidia:
            mock_cerebras.side_effect = ProviderError(
                "Cerebras down", "cerebras", "error"
            )
