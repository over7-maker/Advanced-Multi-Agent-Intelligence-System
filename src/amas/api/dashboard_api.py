...
class DashboardAPI:
    ...
    def __init__(self, orchestrator: UnifiedOrchestratorV2, service_manager: ServiceManager):
        """
    ...
    def _setup_routes(self):
        ...
        def get_system_metrics():
            ...
            try:
                ...
                success_rate = (metrics.get('tasks_completed', 0) / total_tasks * 100) if total_tasks > 0 else 100.0
                
                return jsonify({
                    'totalAgents': metrics.get('active_agents', 0),
                    'activeAgents': len([a for a in self.orchestrator.agents.values() if a.status == 'active']),
            ...
            try:
                ...
                for agent_id, agent in self.orchestrator.agents.items():
                    ...
                    agents_data.append({
                        ...
                        'successRate': round(agent_status.get('metrics', {}).get('success_rate', 0.0) * 100, 1),
                        'lastActive': agent_status.get('last_active', datetime.utcnow().isoformat()),
            ...
            try:
                ...
                for i in range(24):
                    ...
                    performance_data.append({
                        ...
                        'success': success,
                        'failed': failed,
                        'successRate': round((success / tasks) * 100, 1) if tasks > 0 else 0
                    })
            ...
            try:
                ...
                for agent_id, agent in self.orchestrator.agents.items():
                    ...
                    agent_analytics.append({
                        ...
                        'tasksCompleted': metrics.get('tasks_completed', 0),
                        'tasksFailed': metrics.get('tasks_failed', 0),
                        'successRate': round(metrics.get('success_rate', 0.0) * 100, 1),
                        'averageResponseTime': round(metrics.get('average_response_time', 0.0), 2),
        ...
        def get_services_status():
            ...
            try:
                ...
                if self.orchestrator.universal_ai_manager:
                    ai_status = asyncio.run(self.orchestrator.universal_ai_manager.get_status())
                ...
                if self.service_manager.vector_service:
                    services_status['vector_service'] = {
                        'status': 'healthy' if self.service_manager.vector_service.is_initialized else 'unavailable',
                ...
                if self.service_manager.knowledge_graph_service:
                    services_status['knowledge_graph_service'] = {
                        'status': 'healthy' if self.service_manager.knowledge_graph_service.is_initialized else 'unavailable',
...
def create_dashboard_api(orchestrator: UnifiedOrchestratorV2, service_manager: ServiceManager) -> DashboardAPI:
    """
