    ...
    class DummyObserver:
        ...
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            raise RuntimeError(
                ...
                "Install with: pip install watchdog (or equivalent for your environment)"
            )
...
def safe_path(base_dir: Path, user_path: str) -> Path:
    """
    Validate and resolve user path to prevent directory traversal attacks.
    
    Args:
        base_dir: Base directory that all paths must be relative to
        user_path: User-provided path string to validate
    
    Returns:
        Resolved Path object within base_dir
    
    Raises:
        ValueError: If path attempts to escape base_dir (path traversal attack)
    """
