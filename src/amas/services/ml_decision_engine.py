...
class MLDecisionEngine:
    ...
    async def allocate_task(self, task: Task, strategy: DecisionStrategy = DecisionStrategy.ML_OPTIMIZED) -> AllocationDecision:
        ...
        try:
            ...
            if strategy in self.decision_strategies:
                decision = await self.decision_strategies[strategy](task, available_agents)
            else:
            ...
            logger.info(f"Task {task.id} allocated to agent {decision.agent_id} with confidence {decision.confidence:.3f}")
    ...
    async def _ml_optimized_allocation(self, task: Task, available_agents: List[Agent]) -> AllocationDecision:
        """ML-optimized task allocation"""
        try:
            if not self.allocation_model or len(self.historical_data) < self.ml_config["min_training_samples"]:
            ...
            decision = AllocationDecision(
                ...
                estimated_completion_time=datetime.utcnow() + timedelta(minutes=task.estimated_duration),
                ...
                alternative_agents=[agent.id for i, agent in enumerate(available_agents) if i != best_agent_idx],
    ...
    async def _rule_based_allocation(self, task: Task, available_agents: List[Agent]) -> AllocationDecision:
        ...
        try:
            ...
            decision = AllocationDecision(
                ...
                alternative_agents=[agent.id for agent, _ in sorted(agent_scores, key=lambda x: x[1], reverse=True)[1:3]],
                risk_factors=await self._assess_risks(task, best_agent)
            )
    ...
    async def _hybrid_allocation(self, task: Task, available_agents: List[Agent]) -> AllocationDecision:
        ...
        try:
            ...
            if ml_decision.confidence > 0.7:
                ...
                ml_decision.reasoning = f"Hybrid: ML decision with reduced confidence due to uncertainty"
                return ml_decision
    ...
    async def _random_allocation(self, task: Task, available_agents: List[Agent]) -> AllocationDecision:
    ...
    async def _round_robin_allocation(self, task: Task, available_agents: List[Agent]) -> AllocationDecision:
    ...
    async def _prepare_allocation_features(self, task: Task, available_agents: List[Agent]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for agent in available_agents:
                ...
                feature_vector = [
                    self.label_encoders["task_type"].transform([task.type.value])[0] if hasattr(self.label_encoders["task_type"], 'classes_') else 0,
                    task.complexity,
                    task.priority,
                    self.label_encoders["agent_type"].transform([agent.type.value])[0] if hasattr(self.label_encoders["agent_type"], 'classes_') else 0,
                    agent.performance_score,
                    agent.current_load,
                    len(set(task.required_capabilities) & set(agent.capabilities)) / len(task.required_capabilities),
    ...
    async def _assess_risks(self, task: Task, agent: Agent) -> List[str]:
        ...
        try:
            ...
            missing_capabilities = set(task.required_capabilities) - set(agent.capabilities)
            ...
            # Deadline risk
            if task.deadline:
                estimated_completion = datetime.utcnow() + timedelta(minutes=task.estimated_duration)
        ...
        while True:
            try:
                ...
                performance_data = {
                    ...
                    "active_agents": len([a for a in self.agents.values() if a.availability]),
                    "average_load": np.mean([a.current_load for a in self.agents.values()]) if self.agents else 0,
                ...
                cutoff_time = datetime.utcnow() - timedelta(hours=self.ml_config["feature_window"])
                self.performance_history = [
    ...
    async def _calculate_allocation_success_rate(self) -> float:
        ...
        try:
            ...
            successful_allocations = len([d for d in recent_decisions if d["confidence"] > 0.7])
            
            return successful_allocations / len(recent_decisions) if recent_decisions else 0.0
    ...
    async def _train_allocation_model(self, df: pd.DataFrame):
        ...
        try:
            ...
            X = df[["task_type", "task_complexity", "task_priority", "agent_type", "agent_performance", "agent_load"]].copy()
    ...
    async def _train_performance_model(self, df: pd.DataFrame):
        ...
        try:
            # Prepare features and target
            X = df[["task_type", "task_complexity", "agent_performance", "agent_load"]].copy()
    ...
    async def _train_cost_optimization_model(self, df: pd.DataFrame):
        ...
        try:
            # Prepare features and target
            X = df[["task_type", "task_complexity", "agent_type", "agent_performance"]].copy()
            y = df["cost"]
            
            # Encode categorical features
            X["task_type"] = self.label_encoders["task_type"].transform(X["task_type"])
            X["agent_type"] = self.label_encoders["agent_type"].transform(X["agent_type"])
            ...
            logger.info(f"Cost optimization model trained with accuracy: {accuracy:.3f}")
    ...
    async def _train_load_balancing_model(self, df: pd.DataFrame):
        ...
        try:
            # Prepare features and target
            X = df[["agent_type", "agent_performance", "agent_load", "resource_utilization"]].copy()
    ...
    async def _save_models(self):
        ...
        try:
            ...
            if self.allocation_model:
                joblib.dump(self.allocation_model, os.path.join(models_dir, "allocation_model.joblib"))
            
            if self.performance_model:
                joblib.dump(self.performance_model, os.path.join(models_dir, "performance_model.joblib"))
            
            if self.cost_optimization_model:
                joblib.dump(self.cost_optimization_model, os.path.join(models_dir, "cost_optimization_model.joblib"))
            
            if self.load_balancing_model:
                joblib.dump(self.load_balancing_model, os.path.join(models_dir, "load_balancing_model.joblib"))
