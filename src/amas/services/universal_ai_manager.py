...
class UniversalAIManager:
    ...
    def _initialize_providers(self):
        ...
        self.active_providers = [
            pid for pid, config in self.providers.items()
    ...
    def _select_next_provider(self, strategy: str = 'intelligent') -> Optional[str]:
        ...
        if strategy == 'priority':
            ...
            for pid in available:
                config = self.providers[pid]
                success_rate = config.get_success_rate() / 100.0 if config.get_success_rate() > 0 else 0.5
                
                # Faster response time = higher weight
                speed_factor = 1.0 / (config.avg_response_time + 0.1) if config.avg_response_time > 0 else 1.0
            ...
            available_configs.sort(key=lambda x: x[1].avg_response_time if x[1].avg_response_time > 0 else 999)
            ...
            try:
                ...
                if result['success']:
                    ...
                    if config.avg_response_time == 0:
                        ...
                        config.avg_response_time = (config.avg_response_time + result['response_time']) / 2
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.global_stats['total_requests'] > 0:
            success_rate = (self.global_stats['successful_requests'] / 
                          self.global_stats['total_requests']) * 100
    ...
    def get_provider_health(self) -> Dict[str, Any]:
        ...
        for provider_id, config in self.providers.items():
            ...
            health[provider_id] = {
                ...
                'rate_limited': config.rate_limit_until.isoformat() if config.rate_limit_until else None
            }
...
async def test_universal_ai_manager():
    ...
    result = await manager.generate(
        "Say 'Speed test' and nothing else.",
        strategy='fastest'
    )
    ...
    for provider_id, info in health.items():
        ...
        print(f"{status_emoji} {info['name']:25s} | Success Rate: {info['success_rate']:6s} | Avg Time: {info['avg_response_time']:8s} | Status: {info['status']}")
    print()
