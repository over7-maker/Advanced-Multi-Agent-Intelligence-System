...
class AIServiceManager:
    ...
    def _initialize_providers(self):
        ...
        try:
            ...
            logger.info(
                f"Initialized {len([p for p in self.providers.values() if p.enabled])} AI providers"
            )
        ...
        for provider in available_providers:
            try:
                ...
                if response.success:
                    ...
                    logger.warning(
                        f"Provider {provider.value} failed: {response.error}"
                    )
        ...
        for provider in preferred_providers:
            if provider in self._get_available_providers():
                response = await self.generate_response(
                    coding_prompt, preferred_provider=provider
                )
                if response.success:
                    return response
...
Provide a detailed analysis with specific recommendations."""
    ...
    async def improve_code(
        self, code: str, language: str = "python", improvement_type: str = "general"
    ) -> AIResponse:
...
Return complete test code with setup and teardown."""
    ...
    def get_provider_stats(self) -> Dict[str, Any]:
        ...
        for provider, data in self.provider_stats.items():
            ...
            stats[provider.value] = {
                ...
                "last_used": (
                    data["last_used"].isoformat() if data["last_used"] else None
                ),
            }
        return stats
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        healthy_count = sum(
            1 for p in health_status["providers"].values() if p["status"] == "healthy"
        )
