...
class GracefulShutdownService:
            ...
            if asyncio.iscoroutinefunction(service.shutdown_func):
                ...
                shutdown_task = asyncio.create_task(
                    asyncio.get_event_loop().run_in_executor(
                        None, service.shutdown_func
                    )
                )

            # Wait for either completion or timeout
            done, pending = await asyncio.wait(
                [shutdown_task, timeout_task], return_when=asyncio.FIRST_COMPLETED
            )
    ...
    async def _force_shutdown_if_needed(self):
        ...
        if (
            elapsed_time
            > self.config.shutdown_timeout + self.config.force_shutdown_timeout
        ):
            self.logger.error(
                "Force shutdown initiated - graceful shutdown took too long"
            )
    ...
    def _update_shutdown_stats(self, successful: bool, shutdown_time: float):
        ...
        self.shutdown_stats["average_shutdown_time"] = (
            current_avg * (total - 1) + shutdown_time
        ) / total
    ...
    def get_shutdown_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.shutdown_stats,
            ...
            "shutdown_phase": (
                self.shutdown_phase.value if self.shutdown_phase else None
            ),
...
def register_service_for_shutdown(
    ...
    service.register_service(
        name, shutdown_func, priority, timeout, critical, dependencies
    )
...
class ShutdownContext:
    ...
    def __init__(
        self, service_name: str, priority: ServicePriority = ServicePriority.MEDIUM
    ):
    ...
    def __enter__(self):
        ...
        self.service.register_service(self.service_name, self._shutdown, self.priority)
        return self
...
def shutdown_managed(
    ...
    def decorator(func: Callable) -> Callable:
        service = get_graceful_shutdown_service()
        service.register_service(service_name, func, priority, timeout, critical)
        return func
