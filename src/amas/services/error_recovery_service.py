...
class RetryAction(RecoveryAction):
    ...
    async def execute(self, context: ErrorContext) -> bool:
        ...
        self.logger.info(f"Retrying {context.operation} in {delay}s (attempt {context.retry_count + 1})")
    ...
    async def execute(self, context: ErrorContext) -> bool:
        ...
        try:
            ...
            if breaker:
                ...
                self.logger.warning(f"Circuit breaker '{self.circuit_breaker_name}' opened due to errors")
                return True
            else:
                self.logger.error(f"Circuit breaker '{self.circuit_breaker_name}' not found")
                return False
...
class ServiceRestartAction(RecoveryAction):
    ...
    def __init__(self, name: str, config: RecoveryConfig, service_name: str, restart_func: Callable):
        super().__init__(name, config)
    ...
    async def execute(self, context: ErrorContext) -> bool:
        """Restart the service"""
        try:
            self.logger.warning(f"Restarting service '{self.service_name}' due to errors")
...
class ErrorRecoveryService:
    ...
    def _setup_default_recovery_strategies(self):
        ...
        self.add_recovery_strategy(
            ...
            [
                ...
                GracefulDegradationAction("degradation", self.config, self._api_degradation)
            ]
        )
        ...
        self.add_recovery_strategy(
            "memory_error",
            [
                GracefulDegradationAction("degradation", self.config, self._memory_degradation),
                ServiceRestartAction("restart", self.config, "memory_cleanup", self._cleanup_memory)
            ]
        )
    ...
    def add_recovery_strategy(self, error_type: str, actions: List[RecoveryAction]):
        ...
        self.logger.info(f"Added recovery strategy for {error_type} with {len(actions)} actions")
    
    async def handle_error(self, context: ErrorContext) -> bool:
        """Handle error with appropriate recovery strategy"""
        self.logger.error(f"Handling error: {context.error_type} - {context.error_message}")
        ...
        for action in strategy:
            try:
                ...
                if success:
                    self._update_recovery_stats(context.error_type, f"{action.name}_success")
                    self.logger.info(f"Recovery action '{action.name}' succeeded for {context.error_type}")
                    return True
                else:
                    self._update_recovery_stats(context.error_type, f"{action.name}_failed")
                    self.logger.warning(f"Recovery action '{action.name}' failed for {context.error_type}")
            except Exception as e:
                self.logger.error(f"Recovery action '{action.name}' raised exception: {e}")
                self._update_recovery_stats(context.error_type, f"{action.name}_exception")
