...
class AuditLoggingService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.audit_config = {
            ...
            'log_rotation_size': config.get('audit_rotation_size', 100 * 1024 * 1024),  # 100MB
    ...
    async def _initialize_audit_storage(self):
        ...
        try:
            ...
            for compliance in self.compliance_requirements.keys():
                compliance_dir = self.audit_storage_path / f"compliance_{compliance.lower()}"
    ...
    async def log_audit_event(self, event_type: AuditEventType, audit_level: AuditLevel,
                             user_id: str, action: str, resource: str, result: str,
                             details: Dict[str, Any], classification: str = 'unclassified',
                             session_id: str = None, source_ip: str = '127.0.0.1',
                             user_agent: str = 'AMAS-System') -> str:
        """Log an audit event"""
        try:
            ...
            if self.audit_chain:
                event.chain_hash = self._generate_chain_hash(event, self.audit_chain[-1])
            else:
            ...
            logger.info(f"Audit event logged: {event_type.value} - {action} by {user_id}")
    ...
    async def _apply_audit_rules(self, event: AuditEvent):
        ...
        try:
            ...
            applicable_rules = [
                rule for rule in self.audit_rules
        ...
        try:
            ...
            for field in required_fields:
                if not hasattr(event, field) or getattr(event, field) is None:
                    logger.warning(f"Missing required field {field} in audit event {event.event_id}")
    ...
    async def _apply_retention_policy(self, event: AuditEvent, rule: Dict[str, Any]):
        ...
        try:
            retention_days = rule.get('retention_days', self.audit_config['log_retention_days'])
    ...
    async def _write_audit_log(self, event: AuditEvent):
        ...
        try:
            ...
            event_file = self.audit_storage_path / event.event_type.value / f"{event.timestamp.date()}.jsonl"

            with open(event_file, 'a') as f:
                f.write(json.dumps(log_entry) + '\n')

            # Write to general audit log
            general_file = self.audit_storage_path / f"audit_{event.timestamp.date()}.jsonl"
    ...
    async def _cleanup_expired_logs(self):
        ...
        try:
            ...
            cutoff_date = datetime.utcnow() - timedelta(days=self.audit_config['log_retention_days'])

            # Remove expired events from memory
            self.audit_events = {
                event_id: event for event_id, event in self.audit_events.items()
    ...
    def _generate_audit_signature(self, event_id: str, action: str, details: Dict[str, Any]) -> str:
    ...
    def _generate_chain_hash(self, current_event: AuditEvent, previous_event: Optional[AuditEvent]) -> str:
        """Generate chain hash for tamper detection"""
        try:
            if previous_event:
    ...
    async def get_audit_events(self, event_type: AuditEventType = None,
        ...
        try:
            ...
            for i, event in enumerate(self.audit_chain):
                if i > 0:
                    ...
                    if event.chain_hash != expected_hash:
                        ...
                        integrity_status['violations'].append(f"Chain hash mismatch at event {event.event_id}")
            ...
            for event in self.audit_events.values():
                ...
                if event.signature != expected_signature:
                    ...
                    integrity_status['tamper_detected'] = True
                    integrity_status['violations'].append(f"Signature mismatch for event {event.event_id}")
