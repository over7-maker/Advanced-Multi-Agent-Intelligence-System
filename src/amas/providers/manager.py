...
class ProviderManager:
    ...
    def _get_provider_configs(self) -> List[ProviderConfig]:
        ...
        return [
            ...
            ProviderConfig("Anthropic", "ANTHROPIC_API_KEY", required=False, priority=5),
            ProviderConfig("HuggingFace", "HUGGINGFACE_API_KEY", required=False, priority=6),
            ProviderConfig("NVIDIAAI", "NVIDIAAI_API_KEY", required=False, priority=7),
            ProviderConfig("Replicate", "REPLICATE_API_KEY", required=False, priority=8),
            ProviderConfig("TogetherAI", "TOGETHERAI_API_KEY", required=False, priority=9),
            ProviderConfig("Perplexity", "PERPLEXITY_API_KEY", required=False, priority=10),
            ProviderConfig("DeepSeek", "DEEPSEEK_API_KEY", required=False, priority=11),
            ProviderConfig("MistralAI", "MISTRALAI_API_KEY", required=False, priority=12),
    ...
    def _discover_providers(self):
            ...
            if api_key:
                try:
                    ...
                    if provider and provider.is_available():
                        ...
                        self.logger.warning(f"⚠️ {config.name} (API key present but unavailable)")
        ...
        if required_missing:
            self.logger.warning(f"⚠️ Missing required providers: {', '.join(required_missing)}")
        ...
        self.logger.info(f"🎯 {enabled_count} providers enabled out of {len(self.provider_configs)} configured")
    
    def _create_provider(self, config: ProviderConfig, api_key: str) -> Optional[AIProvider]:
    ...
    def validate_environment(self) -> Dict[str, Any]:
        ...
        if validation_result["enabled_providers"] < 2:
            validation_result["warnings"].append("Only 1 provider enabled - consider adding more for redundancy")
        
        if validation_result["enabled_providers"] >= 5:
            validation_result["recommendations"].append("Excellent redundancy! You have 5+ providers enabled")
...
if __name__ == "__main__":
    ...
    if args.status:
        ...
        for name, info in status.items():
            ...
            print(f"{status_icon} {name}: {info['status']} (Priority: {info['priority']})")