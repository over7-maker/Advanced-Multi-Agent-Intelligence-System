...
class CollectiveIntelligenceEngine:
    ...
    def _initialize_agent_specializations(self):
        ...
        self.agent_specializations = {
            "security_expert": [
                "vulnerability_scanning", "penetration_testing", "threat_analysis",
                "security_audit", "compliance_checking", "risk_assessment"
            ],
            "code_analysis": [
                "static_analysis", "code_quality", "performance_optimization",
                "architectural_review", "dependency_analysis", "refactoring"
            ],
            "intelligence_gathering": [
                "osint_research", "social_media_analysis", "domain_investigation",
                "threat_intelligence", "competitive_analysis", "data_mining"
            ],
            "performance_monitor": [
                "system_monitoring", "performance_profiling", "resource_optimization",
                "bottleneck_identification", "capacity_planning", "sla_monitoring"
            ],
            "documentation_specialist": [
                "technical_writing", "api_documentation", "user_guides",
                "knowledge_management", "content_creation", "information_architecture"
            ],
            "testing_coordinator": [
                "test_planning", "quality_assurance", "automation_testing",
                "regression_testing", "performance_testing", "security_testing"
            ],
            "integration_manager": [
                "system_integration", "api_coordination", "workflow_orchestration",
                "service_mesh", "microservices", "event_driven_architecture"
            ]
        }
    ...
    async def record_task_execution(self, task_id: str, task_type: str, target: str,
        ...
        if knowledge_key in self.shared_knowledge:
            ...
            self.shared_knowledge[knowledge_key].last_updated = datetime.now().isoformat()
            ...
            self.shared_knowledge[knowledge_key].effectiveness_score = (avg_success + avg_quality) / 2
        else:
        ...
        for i, agent1 in enumerate(task_pattern.agents_used):
            for agent2 in task_pattern.agents_used[i+1:]:
                if self.learning_graph.has_edge(agent1, agent2):
                    ...
                    self.learning_graph[agent1][agent2]['weight'] = min(1.0, current_weight + 0.1)
                else:
                    # New collaboration
                    self.learning_graph.add_edge(agent1, agent2, weight=0.1, type="collaboration")
    ...
    async def predict_optimal_agent_combination(self, task_type: str, target: str, 
        ...
        for task_pattern, similarity_score in similar_tasks:
            if task_pattern.success_rate > 0.7:  # Only consider successful tasks
                for agent in task_pattern.agents_used:
                    ...
                    score = similarity_score * task_pattern.success_rate * task_pattern.solution_quality
    ...
    async def _find_similar_tasks(self, task_type: str, target: str, 
                                parameters: Dict[str, Any]) -> List[Tuple[TaskPattern, float]]:
        """Find similar tasks from historical data"""
        ...
        for knowledge in self.shared_knowledge.values():
            if knowledge.category == task_type:
                for pattern in knowledge.patterns:
                    ...
                    similarity = self._calculate_task_similarity(current_features, historical_features)
        ...
        # Cache results
        self.task_similarity_cache[cache_key] = similar_tasks[:10]  # Top 10 similar tasks
        
        return similar_tasks[:10]
    
    def _extract_task_features(self, task_type: str, target: str, parameters: Dict[str, Any]) -> np.ndarray:
        """Extract feature vector from task characteristics"""
        
        features = []
        
        # Task type encoding (one-hot)
        task_types = ["security_scan", "code_analysis", "intelligence_gathering", 
                     "performance_analysis", "documentation", "testing", "integration"]
    ...
    def _calculate_task_similarity(self, features1: np.ndarray, features2: np.ndarray) -> float:
    ...
    def _get_default_agent_combination(self, task_type: str) -> List[Tuple[str, float]]:
        ...
        defaults = {
            "security_scan": [("security_expert", 0.9), ("intelligence_gathering", 0.7)],
            "code_analysis": [("code_analysis", 0.9), ("security_expert", 0.6)],
            "intelligence_gathering": [("intelligence_gathering", 0.9), ("security_expert", 0.5)],
            "performance_analysis": [("performance_monitor", 0.9), ("code_analysis", 0.6)],
            "documentation": [("documentation_specialist", 0.9), ("code_analysis", 0.5)],
            "testing": [("testing_coordinator", 0.9), ("code_analysis", 0.7)],
            "integration": [("integration_manager", 0.9), ("performance_monitor", 0.6)]
        }
        
        return defaults.get(task_type, [("code_analysis", 0.5), ("security_expert", 0.5)])
    ...
    async def get_agent_insights(self, agent_id: str) -> List[AgentInsight]:
        ...
        for knowledge in self.shared_knowledge.values():
            for insight in knowledge.insights:
                if insight.agent_id == agent_id or agent_id in insight.applicable_contexts:
                    insights.append(insight)
        
        # Sort by confidence score and validation count
        insights.sort(key=lambda x: (x.confidence_score, x.validation_count), reverse=True)
    ...
    async def recommend_task_optimizations(self, task_type: str, target: str, 
        ...
        if similar_tasks:
            ...
            successful_patterns = [
                pattern for pattern, similarity in similar_tasks 
            ...
            if successful_patterns:
                ...
                best_combo = max(best_combinations.items(), 
    ...
    async def cross_agent_knowledge_transfer(self):
            ...
            for other_agent in self.agent_specializations.keys():
                if agent != other_agent:
                    ...
                    for insight in other_insights:
                        if (insight.confidence_score > 0.8 and 
                            any(context in self.agent_specializations[agent] 
        ...
        self.logger.info(f"✅ Completed {len(transfer_opportunities[:5])} knowledge transfers")
    ...
    async def _execute_knowledge_transfer(self, transfer_opportunity: Dict[str, Any]):
        ...
        transferred_insight = AgentInsight(
            ...
            confidence_score=insight.confidence_score * 0.8,  # Slightly reduce confidence
        ...
        for knowledge in self.shared_knowledge.values():
            if any(context in self.agent_specializations[to_agent] 
...
class CollectiveIntelligenceManager:
    ...
    async def _generate_learning_report(self):
        ...
        self.logger.info(f"📊 Learning Report: "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report", action="store_true", help="Generate insights report")
