...
class PredictiveIntelligenceEngine:
    ...
    async def add_training_data(self, data_type: str, data: Dict[str, Any]):
        ...
        self.training_data[data_type] = pd.concat([self.training_data[data_type], df_row], ignore_index=True)
        
        # Retrain model if we have enough data
        if len(self.training_data[data_type]) >= 50 and len(self.training_data[data_type]) % 20 == 0:
    ...
    def _extract_task_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        for _, row in df.iterrows():
            ...
            task_types = ['security_scan', 'code_analysis', 'intelligence_gathering', 
                         'performance_analysis', 'documentation', 'testing']
    ...
    def _extract_resource_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        for i, row in df.iterrows():
            ...
            if i >= 5:  # Need at least 5 previous points
                ...
                feature_vector.extend([
                    recent_data['cpu_usage'].mean() if 'cpu_usage' in recent_data else 0,
                    recent_data['memory_usage'].mean() if 'memory_usage' in recent_data else 0,
                    recent_data['task_load'].mean() if 'task_load' in recent_data else 0,
                    recent_data['cpu_usage'].std() if 'cpu_usage' in recent_data else 0,
                    recent_data['memory_usage'].std() if 'memory_usage' in recent_data else 0,
                ])
            else:
    ...
    async def predict_task_outcome(self, task_type: str, target: str, 
                                 parameters: Dict[str, Any], 
                                 agents_planned: List[str]) -> TaskOutcomePrediction:
        ...
        optimization_suggestions = self._generate_optimization_suggestions(task_data, predictions)
    ...
    def _identify_risk_factors(self, task_data: Dict[str, Any], 
    ...
    def _generate_optimization_suggestions(self, task_data: Dict[str, Any], 
                                         predictions: Dict[str, float]) -> List[str]:
        ...
        if predictions['success_probability'] < 0.7:
            ...
            if 'security_expert' not in task_data.get('agents_used', []):
                suggestions.append("Adding security expert agent may improve success rate")
        
        # Duration optimizations
        if predictions['estimated_duration'] > 240:  # 4 minutes
            suggestions.append("Consider using 'quick' depth parameter for faster execution")
        ...
        if task_type == 'security_scan' and 'intelligence_gathering' not in agents:
            suggestions.append("Add intelligence gathering agent for enhanced security analysis")
        
        if task_type == 'code_analysis' and 'security_expert' not in agents:
            suggestions.append("Add security expert for comprehensive code security review")
        
        # Parameter optimizations
        params = task_data.get('parameters', {})
        if not params.get('depth'):
            suggestions.append("Specify depth parameter (quick/standard/comprehensive) for better results")
        
        return suggestions
    
    async def predict_system_resources(self, time_horizon_minutes: int = 60) -> SystemResourcePrediction:
        ...
        try:
            ...
            if 'cpu_usage' in self.models and hasattr(self.models['cpu_usage'], 'predict'):
                try:
            ...
            if 'memory_usage' in self.models and hasattr(self.models['memory_usage'], 'predict'):
                try:
            ...
            if 'task_load' in self.models and hasattr(self.models['task_load'], 'predict'):
                try:
        ...
        if predictions['cpu'] > 70:
            scaling_recommendations.append("Consider horizontal scaling for CPU-intensive tasks")
                ...
                try:
                    ...
                    if len(df) > 10:
                        if model_name == 'task_success':
                            ...
                            features = self._extract_task_features(df)
                            target_col = 'execution_time' if model_name == 'task_duration' else 'quality_score'
                    ...
                    self.logger.warning(f"⚠️ Could not calculate accuracy for {model_name}: {e}")
