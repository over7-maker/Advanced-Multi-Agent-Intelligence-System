...
class AIAPIManager:
    ...
    def _setup_apis(self):
        ...
        if os.getenv('CODESTRAL_API_KEY'):
            self.apis['codestral'] = APIConfig(
                ...
                capabilities=['code_analysis', 'vulnerability_detection', 'technical_assessment'],
                priority=2
            )
    ...
    async def _make_openai_request(self, api_name: str, messages: List[Dict], **kwargs) -> Dict:
    ...
    async def _make_cerebras_request(self, api_name: str, messages: List[Dict], **kwargs) -> Dict:
    ...
    async def _make_cohere_request(self, api_name: str, messages: List[Dict], **kwargs) -> Dict:
    ...
    async def _make_chutes_request(self, api_name: str, messages: List[Dict], **kwargs) -> Dict:
    ...
    async def _make_gemini_request(self, api_name: str, messages: List[Dict], **kwargs) -> Dict:
    ...
    async def _make_nvidia_request(self, api_name: str, messages: List[Dict], **kwargs) -> Dict:
    ...
    def _update_health_failure(self, api_name: str, error: str):
        ...
        if health.consecutive_failures >= self.max_consecutive_failures:
            ...
            logger.warning(f"API {api_name} marked as unhealthy after {health.consecutive_failures} consecutive failures")
        
        # Check for rate limiting
        if "rate limit" in error.lower() or "quota" in error.lower():
            health.rate_limit_until = datetime.now() + timedelta(minutes=5)
            logger.warning(f"API {api_name} rate limited until {health.rate_limit_until}")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for api_name, health in self.health_status.items():
            api_status = {
                ...
                'rate_limited_until': health.rate_limit_until.isoformat() if health.rate_limit_until else None
            }
