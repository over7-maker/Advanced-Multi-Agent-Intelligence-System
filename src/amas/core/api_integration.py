...
from ..agents.orchestrator import AgentOrchestrator, BaseAgent, Task, TaskStatus, AgentType
...
class EnhancedOSINTAgent(EnhancedAgent):
    ...
    async def execute_task(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'success': result.success,
                'osint_data': result.result.get('content', '') if result.success else None,
    ...
    def _extract_sources(self, content: str) -> List[str]:
        ...
        lines = content.split('\n')
        for line in lines:
            if any(keyword in line.lower() for keyword in ['source:', 'reference:', 'according to', 'reported by']):
    ...
    def _assess_confidence(self, content: str) -> float:
        ...
        confidence_indicators = [
            'verified', 'confirmed', 'reliable', 'official', 'documented'
        ]
        uncertainty_indicators = [
            'unverified', 'unconfirmed', 'rumor', 'alleged', 'possible'
        ]
    ...
    async def can_handle_task(self, task: Task) -> bool:
        ...
        osint_keywords = [
            ...
            'social media', 'public information', 'threat intelligence'
        ]
...
class EnhancedInvestigationAgent(EnhancedAgent):
    ...
    async def execute_task(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'success': result.success,
                'analysis': result.result.get('content', '') if result.success else None,
                'threat_level': self._assess_threat_level(result.result.get('content', '')),
                'recommendations': self._extract_recommendations(result.result.get('content', '')),
                'api_used': result.api_used,
                'execution_time': result.execution_time,
                'timestamp': datetime.now().isoformat()
            }
    ...
    def _assess_threat_level(self, content: str) -> str:
        ...
        if any(keyword in content_lower for keyword in ['critical', 'severe', 'immediate', 'urgent']):
            return 'CRITICAL'
        elif any(keyword in content_lower for keyword in ['high', 'significant', 'serious']):
            return 'HIGH'
        elif any(keyword in content_lower for keyword in ['medium', 'moderate', 'concerning']):
            return 'MEDIUM'
        else:
            return 'LOW'
    ...
    async def can_handle_task(self, task: Task) -> bool:
        ...
        investigation_keywords = [
            'investigate', 'analysis', 'threat', 'incident',
            'forensics', 'evidence', 'malware', 'attack'
        ]
...
class EnhancedForensicsAgent(EnhancedAgent):
    ...
    def __init__(self, agent_id: str = None):
        super().__init__(
            agent_id or "forensics_001",
            "Enhanced Forensics Agent",
            AgentType.FORENSICS
        )
        self.capabilities = [
            'digital_forensics',
    ...
    async def execute_task(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'success': result.success,
                'forensics_analysis': result.result.get('content', '') if result.success else None,
                'evidence_types': self._identify_evidence_types(result.result.get('content', '')),
    ...
    def _extract_timeline(self, content: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(pattern in line for pattern in ['2024-', '2023-', 'timestamp', 'time:', 'date:']):
    ...
    async def can_handle_task(self, task: Task) -> bool:
        """Check if this agent can handle the task"""
        forensics_keywords = [
            'forensics', 'evidence', 'malware', 'incident',
            'timeline', 'analysis', 'investigation', 'digital'
        ]
...
class EnhancedReportingAgent(EnhancedAgent):
    ...
    async def execute_task(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'executive_summary': self._extract_executive_summary(result.result.get('content', '')),
                'key_findings': self._extract_key_findings(result.result.get('content', '')),
                'recommendations': self._extract_recommendations(result.result.get('content', '')),
                'api_used': result.api_used,
                'execution_time': result.execution_time,
                'timestamp': datetime.now().isoformat()
    ...
    def _extract_executive_summary(self, content: str) -> str:
        ...
        for line in lines:
            if any(keyword in line.lower() for keyword in ['executive summary', 'summary', 'overview']):
    ...
    def _extract_key_findings(self, content: str) -> List[str]:
        ...
        for line in lines:
            if any(keyword in line.lower() for keyword in ['finding', 'discovered', 'identified', 'key']):
    ...
    async def can_handle_task(self, task: Task) -> bool:
        """Check if this agent can handle the task"""
        reporting_keywords = [
            'report', 'summary', 'briefing', 'documentation',
            'synthesis', 'analysis', 'findings', 'recommendations'
        ]
...
class EnhancedAgentOrchestrator(AgentOrchestrator):
    ...
    async def run_enhanced_investigation(
        ...
        try:
            ...
            investigation_result = await self.enhanced_orchestrator.run_investigation_workflow(
                topic=topic,
...
async def run_enhanced_investigation(topic: str, investigation_type: str = "comprehensive"):
    """Run enhanced investigation"""
    return await enhanced_orchestrator.run_enhanced_investigation(topic, investigation_type)
...
async def main():
    ...
    try:
        ...
        print(f"📄 Final report available: {'Yes' if investigation.get('final_report') else 'No'}")
