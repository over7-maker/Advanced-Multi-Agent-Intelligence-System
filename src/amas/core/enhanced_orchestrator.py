...
class EnhancedOrchestrator:
    ...
    def _setup_agent_capabilities(self):
        ...
        self.agent_capabilities = {
            ...
            'code_agent': AgentCapability(
                ...
                task_types=['code_analysis', 'vulnerability_detection', 'technical_assessment'],
    ...
    async def execute_task(
        ...
        agent_cap = self.agent_capabilities.get(agent_type, self.agent_capabilities['general_agent'])
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                ai_response = await self.api_manager.generate_response(
                    prompt=prompt,
                    system_prompt=system_prompt or self._get_system_prompt(agent_type, task_type),
                    task_type=task_type,
                    max_tokens=4000,
                    temperature=0.7,
                    timeout=timeout
                ...
                logger.info(f"Task {task_id} completed successfully using {result.api_used}")
                break
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(f"Task {task_id} attempt {attempt + 1} failed: {e}. Retrying in {wait_time}s...")
...
Focus on accuracy, helpfulness, and professional communication."""
    ...
    async def run_investigation_workflow(
        ...
        for i, phase in enumerate(phases):
            ...
            if context:
                phase['prompt'] = f"Context from previous analysis:\n{context}\n\n{phase['prompt']}"
        ...
        if investigation_results['phases']:
            ...
            if final_phase['result'].success:
                investigation_results['final_report'] = final_phase['result'].result.get('content', '')
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_execution_time = sum(r.execution_time for r in self.results.values()) / total_tasks
...
async def execute_task(task_id: str, task_type: str, prompt: str, **kwargs) -> TaskResult:
...
async def run_investigation(topic: str, investigation_type: str = "comprehensive") -> Dict[str, Any]:
...
async def main():
    ...
    try:
        ...
        print(f"📄 Final report available: {'Yes' if investigation['final_report'] else 'No'}")
