...
from .ai_api_manager import (
    IntelligentAPIManager, APIProvider, TaskType, APIStatus,
...
class APITestingSuite:
    ...
    def __init__(self):
        ...
        logger.info(f"API Testing Suite initialized with {len(self.test_cases)} test cases")
...
What are the potential problems and how would you fix them?"""
                ...
                expected_response_keywords=["zero", "division", "error", "exception", "validation"],
            ...
            TestCase(
                ...
                expected_response_keywords=["logical", "contradiction", "false", "assumption", "premise"],
            ...
            TestCase(
                ...
                expected_response_keywords=["AI", "cybersecurity", "threat", "computer", "security"],
                max_response_time=20.0,
                min_response_length=50
            ),
...
Artificial Intelligence has revolutionized many industries, from healthcare to finance. In cybersecurity, AI systems can detect anomalies, predict threats, and respond to incidents faster than human analysts. However, AI also introduces new vulnerabilities and attack vectors that organizations must consider. The balance between leveraging AI's benefits while mitigating its risks is crucial for modern cybersecurity strategies. Organizations need to implement proper AI governance, ensure data quality, and maintain human oversight of AI systems."""
                ...
                expected_response_keywords=["AI", "cybersecurity", "benefits", "risks", "organizations"],
                max_response_time=15.0,
                min_response_length=50
            ),
            ...
            TestCase(
                ...
                expected_response_keywords=["preparation", "detection", "response", "recovery", "lessons"],
                max_response_time=15.0,
                min_response_length=100
            ),
            ...
            TestCase(
                ...
                expected_response_keywords=["attack", "investigation", "isolation", "forensics", "incident"],
            ...
            TestCase(
                ...
                messages=[{
                    "role": "user",
                    "content": """This is a stress test with a very long prompt. """ * 20 + """Please provide a comprehensive analysis of modern cybersecurity challenges, including threat actors, attack vectors, defensive strategies, emerging technologies, regulatory considerations, and future trends. Be thorough and detailed."""
                }],
                expected_response_keywords=["cybersecurity", "threats", "defense", "technology", "future"],
                max_response_time=45.0,
                min_response_length=500
            )
        ]
    
    async def test_single_provider(self, provider: APIProvider, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            # Determine success
            success = (
                response_time <= test_case.max_response_time and
                response_length >= test_case.min_response_length and
                (not test_case.expected_response_keywords or contains_keywords)
            )
            ...
            if success:
                logger.info(f"✅ {provider.value} passed {test_case.name} in {response_time:.2f}s")
            else:
                logger.warning(f"⚠️ {provider.value} failed {test_case.name}: time={response_time:.2f}s, length={response_length}, keywords={contains_keywords}")
    ...
    async def test_all_providers(self, test_case_ids: List[str] = None) -> Dict[str, List[TestResult]]:
        ...
        logger.info(f"🚀 Starting comprehensive testing of {len(self.api_manager.endpoints)} providers with {len(test_cases_to_run)} test cases")
    ...
    async def run_stress_test(self, concurrent_requests: int = 5) -> Dict[str, Any]:
        ...
        logger.info(f"🔥 Running stress test with {concurrent_requests} concurrent requests")
        ...
        async def stress_request():
            ...
            try:
                start_time = time.time()
                response = await generate_ai_response(
                    messages=basic_test.messages,
                    task_type=basic_test.task_type
                )
                response_time = time.time() - start_time
        ...
        for result in results:
            if isinstance(result, dict):
                ...
                if result['success']:
                    ...
                    stress_results['provider_performance'][provider]['total_time'] += result['response_time']
                else:
                    stress_results['failed_requests'] += 1
        
        # Calculate averages
        if stress_results['successful_requests'] > 0:
            stress_results['average_response_time'] = total_response_time / stress_results['successful_requests']
        
        for provider_stats in stress_results['provider_performance'].values():
            if provider_stats['requests'] > 0:
                provider_stats['average_time'] = provider_stats['total_time'] / provider_stats['requests']
        
        stress_results['end_time'] = datetime.now().isoformat()
        
        logger.info(f"✅ Stress test completed: {stress_results['successful_requests']}/{stress_results['total_requests']} successful")
    ...
    def generate_test_report(self) -> str:
        ...
        successful_tests = sum(sum(1 for r in results if r.success) for results in self.test_results.values())
        ...
        for test_case_id, results in self.test_results.items():
            for result in results:
                ...
                if result.success:
                    ...
                    stats['fastest_response'] = min(stats['fastest_response'], result.response_time)
                    stats['slowest_response'] = max(stats['slowest_response'], result.response_time)
        
        # Calculate averages and format stats
        for provider, stats in provider_stats.items():
            if stats['successful_tests'] > 0:
                stats['average_response_time'] = stats['total_response_time'] / stats['successful_tests']
        ...
        sorted_providers = sorted(provider_stats.items(), key=lambda x: x[1]['success_rate'], reverse=True)
        ...
        for test_case_id, results in self.test_results.items():
            ...
            avg_response_time = sum(r.response_time for r in results if r.success) / len(successful_providers) if successful_providers else 0
        ...
        for provider, stats in top_providers:
            if stats['success_rate'] > 80:
                report_lines.append(f"- **{provider}**: {stats['success_rate']:.1f}% success rate, {stats['average_response_time']:.2f}s avg response")
        ...
        for provider, stats in bottom_providers:
            if stats['success_rate'] < 50:
                report_lines.append(f"- **{provider}**: {stats['success_rate']:.1f}% success rate - investigate connectivity/configuration")
    ...
    async def validate_fallback_system(self) -> Dict[str, Any]:
        ...
        try:
            for provider in self.api_manager.endpoints.keys():
                ...
                try:
                    ...
                    logger.info(f"✅ Fallback successful: {provider.value} → {response.get('provider')}")
        ...
        if validation_results['total_fallbacks_triggered'] > 0:
            validation_results['system_resilience_score'] = (
                ...
                validation_results['total_fallbacks_triggered'] * 100
            )
        
        validation_results['end_time'] = datetime.now().isoformat()
        
        logger.info(f"✅ Fallback validation completed: {validation_results['successful_fallbacks']}/{validation_results['total_fallbacks_triggered']} successful")
...
async def run_comprehensive_validation():
    ...
    with open('artifacts/api_test_results.json', 'w') as f:
        json.dump({
            ...
            'test_results': {k: [r.__dict__ for r in v] for k, v in test_results.items()},
