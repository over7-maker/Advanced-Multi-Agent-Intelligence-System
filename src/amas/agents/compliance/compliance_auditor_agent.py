...
class ComplianceAuditorAgent:
    ...
    async def run_compliance_audit(
        self, framework: ComplianceFramework, scope: Dict[str, Any] = None
    ) -> ComplianceReport:
        ...
        try:
            ...
            logger.info(
                f"Compliance audit completed: {compliance_level.value} ({overall_score:.1f}%)"
            )
    ...
    async def _run_compliance_check(
        self, rule: ComplianceRule, scope: Dict[str, Any] = None
    ) -> ComplianceCheck:
    ...
    async def _generate_audit_summary(
        self, checks: List[ComplianceCheck], overall_score: float
    ) -> str:
        """Generate audit summary"""
        try:
            total_checks = len(checks)
            compliant_checks = len(
                [c for c in checks if c.status == ComplianceLevel.COMPLIANT]
            )
            non_compliant_checks = len(
                [c for c in checks if c.status == ComplianceLevel.NON_COMPLIANT]
            )
            partially_compliant_checks = len(
                [c for c in checks if c.status == ComplianceLevel.PARTIALLY_COMPLIANT]
            )
        ...
        try:
            ...
            for rec in unique_recommendations:
                ...
                if any(
                    ...
                    for keyword in ["critical", "immediate", "urgent", "security"]
                ):
                    high_risk_recommendations.append(rec)
                elif any(
                    keyword in rec.lower()
                    for keyword in ["important", "review", "update", "implement"]
                ):
        ...
        try:
            if framework:
                ...
                return {
                    ...
                    "automated_rules": len(
                        [r for r in framework_rules if r.automated_check]
                    ),
                    "risk_distribution": {
                        level.value: len(
                            [r for r in framework_rules if r.risk_level == level]
                        )
                ...
                return {
                    ...
                    "automated_rules": len(
                        [r for r in self.compliance_rules.values() if r.automated_check]
                    ),
                    "frameworks": {
                        framework.value: {
                            "rules": len(
                                [
                                    r
                                    for r in self.compliance_rules.values()
    ...
    async def generate_compliance_report(
        self, framework: ComplianceFramework, format: str = "json"
        ...
        try:
            ...
            recent_reports = [
                ...
                if (
                    ...
                    and report.generated_at > datetime.utcnow() - timedelta(days=7)
                )
            ]
