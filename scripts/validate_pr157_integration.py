...
class IntegrationValidator:
    ...
    async def check_async_fixtures(self) -> Dict[str, any]:
        ...
        cmd = ["python", "-m", "pytest", "tests/conftest.py", "-v", "-W", "error::RuntimeWarning", "--tb=short"]
    ...
    async def check_task_structure(self) -> Dict[str, any]:
        ...
        if returncode == 0 and stdout.strip():
            ...
            check_result["message"] = f"Found {len(old_sigs)} old submit_task signatures"
    ...
    async def check_imports(self) -> Dict[str, any]:
        ...
        if check_result["details"]["missing_imports"]:
            check_result["status"] = "FAIL"
            check_result["message"] = f"Missing {len(check_result['details']['missing_imports'])} critical imports"
