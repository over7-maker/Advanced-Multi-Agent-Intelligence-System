...
class MultiAgentOrchestrator:
    ...
    def __init__(self):
        ...
        if not self.api_key or self.api_key == 'your_openrouter_api_key_here':
            raise ValueError("No valid OpenRouter API key found. Please configure in .env file")
    ...
    async def analyze_with_agent(self, agent_id: str, files_content: Dict[str, str]) -> Dict[str, Any]:
...
Format your response as JSON with: coverage_gaps, test_scenarios, recommendations"""
            ...
            response = await self.client.chat.completions.create(
                model=agent['model'],
                messages=[
                    {"role": "system", "content": f"You are the {agent['name']}. {agent['description']}"},
    ...
    async def generate_improvement_report(self, analysis_results: Dict[str, Any]) -> str:
...
"""
        ...
        for agent_id, result in analysis_results['agent_results'].items():
            ...
            if result['status'] == 'success':
                ...
                report += f"❌ Analysis failed: {result.get('error', 'Unknown error')}\n\n"
...
async def main():
    ...
    try:
        ...
        # Save raw results
        results_file = f"multi_agent_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
