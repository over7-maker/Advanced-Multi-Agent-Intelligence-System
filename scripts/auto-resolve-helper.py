...
class ConflictResolver:
    def __init__(self):
        ...
        self.import_pattern = re.compile(r'^\s*(from\s+[\w\.]+\s+import\s+.*|import\s+[\w\.]+.*)', re.MULTILINE)
        ...
        try:
            ...
            for conflict in conflicts:
                ...
                if 'import' in conflict or '__all__' in conflict:
                    head_imports, branch_imports = self.extract_imports_from_conflict(conflict)
                    
                    if head_imports or branch_imports:
                        merged_imports = self.merge_imports(head_imports, branch_imports)
                        replacement = '\n'.join(merged_imports)
                        resolved_content = resolved_content.replace(conflict, replacement)
                        print(f"Merged {len(head_imports)} + {len(branch_imports)} imports")
                    ...
                    if '__all__' in conflict:
                        ...
                        if all_exports:
                            ...
                            resolved_content = resolved_content.replace(conflict, all_list)
