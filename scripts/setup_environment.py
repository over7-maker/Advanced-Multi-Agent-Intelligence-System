...
class EnvironmentSetup:
    ...
    def check_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dependencies = [
                'pip', 'python', 'git', 'docker', 'docker-compose'
            ]
            ...
            for dep in dependencies:
                try:
                    ...
                    if result.returncode == 0:
                        results[dep] = {'installed': True, 'version': result.stdout.strip()}
    ...
    def _generate_setup_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_deps = [dep for dep, status in deps.items()
                           if not status.get('installed', False)]
            if missing_deps:
                summary['issues'].append(f"Missing dependencies: {', '.join(missing_deps)}")
                summary['setup_complete'] = False
            ...
            missing_secrets = [secret for secret, status in secrets.items()
                              if status == 'missing']
            if missing_secrets:
                summary['recommendations'].append(f"Set up GitHub Secrets: {', '.join(missing_secrets)}")

            # Generate recommendations
            if summary['setup_complete']:
                summary['recommendations'].append("Environment setup is complete!")
                summary['recommendations'].append("Run 'python scripts/setup_ai_integration.py' to test AI integration")
            else:
                summary['recommendations'].append("Fix the issues above before proceeding")
...
def main():
    ...
    parser.add_argument('--output', default='environment_setup_report.json', help='Output file for setup report')
