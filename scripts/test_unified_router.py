...
async def test_router():
    ...
    for model_id, info in status['models'].items():
        print(f"{model_id}: {info['provider']} - {info['model']} (Available: {info['available']})")
    ...
    if working_models > 0:
        ...
        try:
            response, model_used, metadata = await router.complete(messages, max_tokens=10)
    ...
    try:
        agent_messages = [
            ...
            {"role": "user", "content": "Is Python a good language? Answer in 5 words or less."}
        ]
