...
class SimpleWorkflowTest:
    ...
    def _generate_test_summary(
        ...
        try:
            ...
            existing_files = len([f for f in file_results.values() if f])
    ...
    def _generate_test_recommendations(
        ...
        if existing_files < 4:
            recommendations.append(
                f"Only {existing_files} required files exist. Check file implementations."
            )
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="simple_workflow_test_report.json",
