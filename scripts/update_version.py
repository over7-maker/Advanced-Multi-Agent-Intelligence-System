...
class AIVersionManager:
    ...
    def __init__(self):
        ...
        self.version_pattern = re.compile(r'^v?(\d+)\.(\d+)\.(\d+)(?:-([a-zA-Z0-9.-]+))?$')
    ...
    def update_version_in_file(self, file_path: Path, pattern: str, old_version: str, new_version: str) -> bool:
        ...
        try:
            ...
            # Replace version
            new_content = re.sub(pattern, lambda m: m.group(0).replace(old_version, new_version), content)
        ...
        for file_path, pattern in self.version_files.items():
            ...
            if path.exists():
                ...
                if current_version:
                    ...
                    success = self.update_version_in_file(path, pattern, current_version, new_version)
...
def main():
    ...
    parser.add_argument('--type', default='minor', choices=['major', 'minor', 'patch', 'prerelease'],
                       help='Version bump type')
    parser.add_argument('--auto-bump', action='store_true', 
                       help='Automatically bump version based on type')
    parser.add_argument('--dry-run', action='store_true',
                       help='Show what would be changed without making changes')
    parser.add_argument('--output', default='VERSION_INFO.md',
        ...
        if args.auto_bump:
            ...
            if pyproject_path.exists():
                current_version, _ = manager.find_version_in_file(
                    pyproject_path, 
                    manager.version_files['pyproject.toml']
                )
            ...
            print(f"📈 Auto-bumping: {current_version} → {target_version} ({args.type})")
        else:
