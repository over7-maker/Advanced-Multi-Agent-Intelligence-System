...
class APIConfigValidator:
    ...
    def generate_config_template(self) -> str:
        ...
        template_lines = [
            "# AMAS API Configuration Template",
        ...
        for provider, config in self.api_providers.items():
            ...
            if not result.get("valid", False):
                template_lines.append(
                    f"{config['env_key']}=your_{provider}_api_key_here"
                )
            else:
                template_lines.append(
                    f"# {config['env_key']} is already configured and valid"
                )
            template_lines.append("")
    ...
    def print_summary(self):
        ...
        invalid_count = sum(
            1 for r in self.results.values() if r.get("status") == "invalid"
        )
        missing_count = sum(
            1 for r in self.results.values() if r.get("status") == "missing"
        )
        ...
        for provider, result in self.results.items():
            ...
            print(
                f"{status_symbol} {provider.ljust(15)} - {result.get('message', 'Unknown')}"
            )
