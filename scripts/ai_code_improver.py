...
class AICodeImprover:
    ...
    async def improve_file(self, file_path: str, improvement_type: str = "general") -> Dict[str, Any]:
        ...
        try:
            ...
            improved_content = self._extract_code_from_response(improved_response.content, language)
            
            # Generate analysis of improvements
            analysis_response = await self.ai_service.analyze_code(improved_content, language)
            
            return {
                'file_info': file_info,
                'original_content': original_content,
                'improved_content': improved_content,
                'improvement_analysis': analysis_response.content if analysis_response.success else analysis_response.error,
    ...
    async def improve_directory(self, directory: str, output_dir: str, 
        ...
        try:
            ...
            # Generate summary
            results['summary'] = await self._generate_improvement_summary(results['improvements'])
    ...
    async def _generate_improvement_summary(self, improvements: List[Dict[str, Any]]) -> Dict[str, Any]:
...
async def main():
    ...
    parser.add_argument('--improvement-type', default='general',
                      choices=['general', 'performance', 'security', 'readability', 'maintainability'],
                      help='Type of improvement to apply')
    parser.add_argument('--extensions', nargs='+', default=['.py', '.js', '.ts'],
                      help='File extensions to improve')
    parser.add_argument('--report', default='improvement_report.json', help='Report file')
    ...
    try:
        ...
        if args.files:
            ...
            for file_path in args.files:
                logger.info(f"Improving {file_path}")
                improvement = await improver.improve_file(file_path, args.improvement_type)
                results['improvements'].append(improvement)
                if 'improved_content' in improvement:
                    results['files_improved'] += 1
            
            # Generate summary
            results['summary'] = await improver._generate_improvement_summary(results['improvements'])
        ...
        logger.info(f"Improvement complete. {results['files_improved']} files improved.")
