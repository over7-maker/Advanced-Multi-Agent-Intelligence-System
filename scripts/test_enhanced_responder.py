...
from ai_issues_responder_v2 import EnhancedAIIssuesResponder, IssueType, Priority, Sentiment
...
class EnhancedResponderTestSuite:
    ...
    async def test_database_initialization(self):
        ...
        try:
            ...
            if missing_tables:
                ...
                self.add_test_result(test_name, True, "All database tables created successfully", duration)
                
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Database test failed: {e}", duration)
        ...
        try:
            ...
            for text, expected_lang in test_cases:
                ...
                if detected_lang == expected_lang:
                    ...
                    logger.warning(f"Language detection mismatch: expected {expected_lang}, got {detected_lang}")
            
            duration = time.time() - start_time
            success_rate = success_count / len(test_cases)
            
            if success_rate >= 0.8:  # 80% success rate threshold
                self.add_test_result(test_name, True, f"Success rate: {success_rate:.1%}", duration)
            else:
                self.add_test_result(test_name, False, f"Low success rate: {success_rate:.1%}", duration)
                
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Language detection test failed: {e}", duration)
    ...
    async def test_caching_system(self):
        ...
        try:
            ...
            if cached_result is not None:
                self.add_test_result(test_name, False, "Cache should be empty initially")
                return
            ...
            # Cache the results
            self.responder._cache_analysis(issue_number, title, content_hash, test_analysis, test_response)
            ...
            if cached_result and cached_result['analysis']['success']:
                duration = time.time() - start_time
                self.add_test_result(test_name, True, "Caching system working correctly", duration)
            else:
                self.add_test_result(test_name, False, "Cache retrieval failed")
                
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Caching test failed: {e}", duration)
    ...
    async def test_rate_limiting(self):
        ...
        try:
            ...
            if not self.responder._check_rate_limit('test_service'):
                self.add_test_result(test_name, False, "Rate limiting should allow initial requests")
                return
            ...
            if not self.responder._check_rate_limit('test_service'):
                self.add_test_result(test_name, False, "Should still be within rate limit")
                return
            ...
            duration = time.time() - start_time
            self.add_test_result(test_name, True, "Rate limiting working correctly", duration)
            
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Rate limiting test failed: {e}", duration)
    ...
    async def test_fallback_analysis(self):
        ...
        try:
            test_cases = [
                ("Bug in login system", "The application crashes when I try to log in", IssueType.BUG),
                ("Add dark mode feature", "Please add a dark mode toggle to the settings", IssueType.FEATURE_REQUEST),
                ("How to install?", "I need help installing this software", IssueType.QUESTION),
                ("Update documentation", "The README needs to be updated with new examples", IssueType.DOCUMENTATION)
            ]
            ...
            for title, body, expected_type in test_cases:
                ...
                if result['success'] and result['analysis_data']['issue_type'] == expected_type.value:
                    success_count += 1
                else:
                    logger.warning(f"Fallback analysis mismatch for '{title}': expected {expected_type.value}, got {result['analysis_data']['issue_type']}")
            ...
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Fallback analysis test failed: {e}", duration)
    ...
    async def test_performance_metrics(self):
        ...
        try:
            ...
            if 'error' in report:
                self.add_test_result(test_name, False, f"Performance report error: {report['error']}")
                return
            
            # Check if report contains expected data
            if 'generated_at' in report and 'cached_issues' in report:
                duration = time.time() - start_time
                self.add_test_result(test_name, True, "Performance metrics working correctly", duration)
            else:
                self.add_test_result(test_name, False, "Performance report missing expected fields")
                
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Performance metrics test failed: {e}", duration)
    ...
    async def test_follow_up_scheduling(self):
        ...
        try:
            ...
            if result and result[0] == test_issue:
                duration = time.time() - start_time
                self.add_test_result(test_name, True, "Follow-up scheduling working correctly", duration)
            else:
                self.add_test_result(test_name, False, "Follow-up not found in database")
                
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Follow-up scheduling test failed: {e}", duration)
    ...
    async def test_template_system(self):
        ...
        try:
            ...
            required_types = [IssueType.BUG, IssueType.FEATURE_REQUEST, IssueType.QUESTION]
            missing_templates = []
            ...
            if missing_templates:
                self.add_test_result(test_name, False, f"Missing templates: {missing_templates}")
                return
            ...
            for issue_type, template in en_templates.items():
                ...
                if missing_keys:
                    self.add_test_result(test_name, False, f"Template {issue_type} missing keys: {missing_keys}")
                    return
            
            duration = time.time() - start_time
            self.add_test_result(test_name, True, "Template system working correctly", duration)
            
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Template system test failed: {e}", duration)
    ...
    async def test_enhanced_processing_flow(self):
        ...
        try:
            ...
            if missing_fields:
                self.add_test_result(test_name, False, f"Missing result fields: {missing_fields}")
                return
            
            if result.get('success'):
                self.add_test_result(test_name, True, f"Processing successful in {duration:.2f}s", duration)
            else:
                ...
                if 'fallback' in error.lower() or 'analysis failed' in error.lower():
                    self.add_test_result(test_name, True, f"Expected fallback behavior: {error}", duration)
                else:
                    self.add_test_result(test_name, False, f"Unexpected error: {error}", duration)
            
        except Exception as e:
            duration = time.time() - start_time
            self.add_test_result(test_name, False, f"Enhanced processing test failed: {e}", duration)
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if total_duration > 30:
            report['recommendations'].append("Consider optimizing test performance for faster execution")
        
        if report['test_summary']['success_rate'] < 0.8:
            report['recommendations'].append("Success rate below 80% - investigate system stability")
