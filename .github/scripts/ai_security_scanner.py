...
class AISecurityScanner:
    ...
    def _is_pattern_definition_file(self, content: str, file_path: str) -> bool:
        ...
        if any(
            ...
            pattern_count = sum(
                1 for indicator in pattern_indicators if indicator in content
            )
    ...
    def _is_pattern_definition_line(self, line: str, context_lines: list) -> bool:
        ...
        if any(
            "{" in context_line and "}" in context_line
            for context_line in context_lines
        ):
        ...
        if re.search(r"""['"]\s*r?['"]\s*[a-zA-Z_\\.*+()]+\s*['"]\s*['"]\s*,?""", line):
            ...
            if any(
                ...
                for keyword in ["sql", "xss", "injection", "patterns", "vulnerabil"]
            ):
                return True
        ...
        for vuln_type, vuln_info in vuln_patterns.items():
            for pattern in vuln_info["patterns"]:
                ...
                for match in matches:
                    ...
                    if self._is_pattern_definition_line(
                        lines[line_num - 1], context_lines
                    ):
                        print(
                            f"🔍 Skipping pattern definition line {line_num} in {file_path}"
                        )
                        continue
            ...
            if result["secrets"] or result["vulnerabilities"]:
                ...
                if result["secrets"]:
                    ...
                    for secret in result["secrets"]:
                        report.append(
                            f"- **{secret['type']}** (Line {secret['line']}): `{secret['match']}`"
                        )
                    report.append("")
                ...
                if result["vulnerabilities"]:
                    ...
                    for vuln in result["vulnerabilities"]:
                        report.append(
                            f"- **{vuln['description']}** (Line {vuln['line']})"
                        )
                        report.append(f"  ```\n{vuln['code_snippet']}\n  ```")
                    report.append("")
    ...
    def run(self):
        ...
        for file_path in self.changed_files:
            ...
            if (secrets or vulnerabilities) and self.ai_clients:
                ai_analysis = self.analyze_security_with_ai(
                    file_path, content, secrets, vulnerabilities
                )
