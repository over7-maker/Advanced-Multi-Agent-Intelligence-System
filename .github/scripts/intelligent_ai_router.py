...
class IntelligentAIRouter:
    ...
    def route_task(self, task_type: str, task_description: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """Route task to the most appropriate AI agent"""
        self.logger.info(f"🎯 Routing task: {task_type}")
        
        # Get routing rules for this task type
        routing_rule = self.routing_rules.get(task_type, self.routing_rules['code_analysis'])
    ...
    def _find_best_model(self, routing_rule: Dict[str, List[str]], task_type: str) -> Optional[Dict[str, Any]]:
    ...
    def _execute_task(self, model: Dict[str, Any], task_description: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """Execute task with selected model"""
        try:
            # Create specialized prompt
            prompt = self._create_task_prompt(task_description, context, model['specialization'])
            
            response = model['client'].chat.completions.create(
                model=model['model'],
                messages=[
                    {"role": "system", "content": f"You are a specialized AI agent focused on {model['specialization']}"},
    ...
    def _create_task_prompt(self, task_description: str, context: Dict[str, Any], specialization: str) -> str:
    ...
    def _update_performance_metrics(self, model_name: str, result: Dict[str, Any]):
        ...
        if result.get('success', False):
            self.performance_metrics['success_rate'][model_name] = min(1.0, 
                self.performance_metrics['success_rate'][model_name] + 0.1)
        else:
            self.performance_metrics['success_rate'][model_name] = max(0.0,
                self.performance_metrics['success_rate'][model_name] - 0.1)
    ...
    def get_routing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'available_models': len([c for c in self.ai_clients if self._is_model_available(c)]),
    ...
    def _find_best_performing_model(self, task_type: str) -> Optional[str]:
        ...
        for model_name, success_rate in self.performance_metrics['success_rate'].items():
