...
class AIAdaptivePromptImprovement:
                    ...
                    for file in files:
                        if file.endswith('.py'):
                            ...
                            try:
                                ...
                                prompt_patterns = self._extract_prompts_from_content(content, file_path)
    ...
    def _extract_prompts_from_content(self, content: str, file_path: str) -> List[Dict[str, Any]]:
        ...
        for pattern in prompt_patterns:
            ...
            for match in matches:
                ...
                prompts.append({
                    ...
                    'type': 'system_prompt' if 'You are' in prompt_text else 'user_prompt'
                })
        
        return prompts
    
    async def analyze_prompt_effectiveness(self, prompts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        optimization_recommendations = await self.call_agent(optimization_agent, optimization_prompt, prompt_analysis)
        if not optimization_recommendations:
            optimization_recommendations = "Optimization recommendations failed - using analysis only"
        ...
        strategic_improvements = await self.call_agent(strategy_agent, strategy_prompt, optimization_recommendations)
        if not strategic_improvements:
            strategic_improvements = "Strategic improvements failed - review optimization recommendations"
        ...
        technical_implementation = await self.call_agent(technical_agent, technical_prompt, strategic_improvements)
        if not technical_implementation:
            technical_implementation = "Technical implementation failed - review strategic improvements"
