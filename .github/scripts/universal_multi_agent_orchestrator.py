...
class UniversalMultiAgentOrchestrator:
    ...
    async def orchestrate_investigation(
        ...
        if any(keyword in topic.lower() for keyword in ['code', 'software', 'application', 'vulnerability']):
    ...
    async def run(self, topic: str = None, strategy: str = 'intelligent'):
        ...
        print(f"📊 Phases completed: {len([p for p in investigation['phases'] if p['result']['success']])}/{len(investigation['phases'])}")
...
async def main():
    ...
    parser.add_argument('--strategy', type=str, default='intelligent',
