...
class SimpleAMASInteractiveCLI:
    ...
    def setup_signal_handlers(self):
        ...
        def signal_handler(signum, frame):
            self.console.print(f"\n🛑 Received signal {signum}. Shutting down gracefully...", style="yellow")
            self.running = False
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    def _classify_intent(self, command: str) -> str:
        ...
        if any(word in command_lower for word in ["scan", "security", "vulnerability", "audit"]):
            return "security_scan"
        elif any(word in command_lower for word in ["analyze", "code", "review", "quality"]):
            return "code_analysis"
        elif any(word in command_lower for word in ["research", "investigate", "intelligence", "osint"]):
            return "intelligence_gathering"
        elif any(word in command_lower for word in ["monitor", "performance", "optimize"]):
        ...
        elif any(word in command_lower for word in ["incident", "emergency", "respond"]):
    ...
    def _extract_target(self, command: str) -> str:
        ...
        # Look for URLs
        url_match = re.search(r'https?://[^\s]+|www\.[^\s]+|[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}', command)
        if url_match:
    ...
    def _get_agents_for_intent(self, intent: str) -> List[str]:
        ...
        agent_mapping = {
            ...
            "intelligence_gathering": ["Intelligence Gathering Agent", "Security Expert Agent"],
            "performance_monitoring": ["Performance Monitor Agent", "Code Analysis Agent"],
            "documentation_generation": ["Documentation Specialist Agent", "Code Analysis Agent"],
            "testing_coordination": ["Testing Coordinator Agent", "Code Analysis Agent"],
            "threat_analysis": ["Security Expert Agent", "Intelligence Gathering Agent"],
            "incident_response": ["Security Expert Agent", "Intelligence Gathering Agent", "Integration Manager Agent"]
        }
        
        return agent_mapping.get(intent, ["Code Analysis Agent"])
    
    async def _execute_with_progress(self, task: Task, task_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with Progress(
            ...
            progress_task = progress.add_task(
                f"Executing {task.intent} task...", 
                total=100
            )
            
            try:
                # Phase 1: Agent initialization (10%)
                progress.update(progress_task, completed=10, 
                              description="Initializing AI agents...")
                await asyncio.sleep(0.5)
                
                # Phase 2: Data collection (30%)
                progress.update(progress_task, completed=20, 
                              description="Collecting intelligence data...")
                await asyncio.sleep(1.0)
                
                # Phase 3: Analysis (50%)
                progress.update(progress_task, completed=50, 
                              description="Performing AI analysis...")
                await asyncio.sleep(1.5)
                
                # Phase 4: Agent coordination (80%)
                progress.update(progress_task, completed=80, 
    ...
    def _generate_task_results(self, task: Task, task_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intent == "security_scan":
            results.update({
                ...
                "headers_analysis": {
                    ...
                    "present_headers": ["X-XSS-Protection", "X-Content-Type-Options"]
                },
                "recommendations": [
    ...
    def _create_task_summary_table(self, task: Task) -> Table:
        ...
        if task.results:
            if "security_score" in task.results:
                ...
                table.add_row("Code Quality", f"{task.results['code_quality_score']}/100")
    ...
    def _create_detailed_results_panel(self, task: Task) -> Panel:
        ...
        if "security_score" in results:
            ...
            content.append(f"• Vulnerabilities: {vulns.get('critical', 0)} Critical, "
        ...
        # Intelligence gathering results
            ...
            content.append(f"• Threat Level: {results.get('threat_level', 'unknown').title()}")
            ...
            content.append(f"• Analysis Complete: {results.get('analysis_complete', False)}")
    ...
    def _display_agent_activity(self, task: Task):
        ...
        for agent in task.agents_involved:
            activity_table.add_row(
                agent,
                "✅ Completed",
                f"Contributed to {task.intent} analysis"
            )
...
- Use quotes for multi-word targets
    ...
    def show_status(self):
        ...
        status_table.add_row("🔒 Security Layer", "✅ Hardened", "0 vulnerabilities detected")
        ...
        status_table.add_row("🗣️ NLP Engine", "✅ Ready", "Natural language processing active")
        
        # Task management
        active_tasks = len([t for t in self.tasks.values() if t.status == TaskStatus.RUNNING])
        completed_tasks = len([t for t in self.tasks.values() if t.status == TaskStatus.COMPLETED])
        
        status_table.add_row("📋 Task Manager", "✅ Active", f"{active_tasks} active, {completed_tasks} completed")
    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                self.console.print(f"📊 Confidence: {task_config['confidence']:.1%}", style="yellow")
